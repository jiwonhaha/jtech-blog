...
Where:
- $$x \in \mathbb{R}^3$$ is the 3D spatial coordinate  
- $$d \in \mathbb{R}^2$$ is the viewing direction  
- $$(r, g, b)$$ is the emitted RGB color  
- $$\sigma$$ is the volume density at point $$x$$  
...
Instead of using the raw coordinate, NeRF encodes each dimension with a set of sinusoidal functions:
